...
<div class="max-w-7xl mx-auto space-y-8">
    ...
    <div class="rounded-3xl border border-slate-200 bg-white shadow-[0_25px_45px_-35px_rgba(15,23,42,0.3)]">
        ...
        <form method="GET" class="flex flex-col gap-3 border-b border-slate-200/70 px-6 py-5 md:grid md:grid-cols-[minmax(0,2fr)_repeat(4,minmax(0,1fr))] md:items-center">
                <div class="relative">
            ...
            <select name="role" class="h-11 min-w-[9rem] rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-600 focus:border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-100">
            ...
            <select name="status" class="h-11 min-w-[9rem] rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-600 focus:border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-100">
                <option value="">Semua Status</option>
                <option value="active" {{ request('status')==='active' ? 'selected' : '' }}>Aktif</option>
                <option value="inactive" {{ request('status')==='inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                <option value="alumni" {{ request('status')==='alumni' ? 'selected' : '' }}>Alumni</option>
            </select>
            <select name="tingkatan" class="h-11 min-w-[9rem] rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-600 focus:border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-100">
                <option value="">Semua Tingkatan</option>
                @foreach(($tingkatanOptions ?? []) as $tingkatan)
                    <option value="{{ $tingkatan }}" {{ (string)request('tingkatan') === (string)$tingkatan ? 'selected' : '' }}>Tingkatan {{ $tingkatan }}</option>
                @endforeach
            </select>
            <div class="flex items-center justify-end gap-2 whitespace-nowrap">
                <button type="submit" class="inline-flex items-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-500 hover:border-indigo-200 hover:text-indigo-600">Terapkan</button>
                <a href="{{ route('admin.users.index') }}" class="inline-flex items-center rounded-2xl border border-transparent bg-slate-100 px-4 py-2 text-sm text-slate-500 hover:bg-slate-200">Reset</a>
            </div>
        </form>
